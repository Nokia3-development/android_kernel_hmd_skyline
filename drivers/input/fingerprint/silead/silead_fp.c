...
// Modify by yuli for QN7026A-155 on 20240226 begin
static char fp_sn[PROC_VND_ID_LEN];

static struct platform_device fpsn_device = {
    .name = "fingerprint",
    .id = -1,
};
...
static DEVICE_ATTR(module_id, 0644, module_id_show, NULL);
static struct attribute *moduleid_sysfs_attrs[] = {
    &dev_attr_module_id.attr,
    NULL,
};

static const struct attribute_group moduleid_group = {
    .attrs = moduleid_sysfs_attrs,
};

int fpsn_node_init(struct platform_device *fpsn_device)
{
    int ret = 0;
    ret = platform_device_register(fpsn_device);
    if (ret != 0) {
        LOG_MSG_DEBUG(ERR_LOG, "[%s] fpsn_device create fail.\n", __func__);
        return ret;
    }
    ret = sysfs_create_group(&fpsn_device->dev.kobj, &moduleid_group);
    if (ret != 0) {
        LOG_MSG_DEBUG(ERR_LOG, "[%s] module_id create fail.\n", __func__);
        platform_device_unregister(fpsn_device);
        return ret;
    }
    return 0;
}
// Modify by yuli for QN7026A-155 on 20240226 end
...
silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    // Modify by yuli for QN7026A-155 on 20240226 begin
    case SIFP_IOC_SYS_NODE:
        if (arg) {
            if (copy_from_user(fp_sn, (void __user *)arg, PROC_VND_ID_LEN)) {
                LOG_MSG_DEBUG(ERR_LOG, "[SIFP_IOC_SYS_NODE] copy_from fail\n");
                retval = -EFAULT;
                break;
            }
        }
...
static int silfp_probe(struct platform_device *spi)
{
    ...
    // Modify by yuli for QN7026A-155 on 20240226 begin
    if (fpsn_node_init(&fpsn_device)) {
        LOG_MSG_DEBUG(ERR_LOG, "[%s] fpsn_node_init fail.\n", __func__);
    } else {
        LOG_MSG_DEBUG(ERR_LOG, "[%s] fpsn_node_init success.\n", __func__);
    }
    // Modify by yuli for QN7026A-155 on 20240226 end
