...
static int sec_nfc_open(struct inode *inode, struct file *file)
{
    ...
    if (first_try == 1) {
        ...
        if (pdata->firm) {
            ...
            if (ret) {
                pr_err("failed to get gpio firm\n");
            } else {
                pr_err("success to get gpio firm\n");
            }
...
static int sec_nfc_suspend(struct device *dev)
{
    ...
    if(value > 0){
        // Modify by yusen for TCT-781 on 20240123 begin
        // pr_err("%s: hal is still active, block suspend.\n", __func__);
        // ret = -EPERM;
        // Modify by yusen for TCT-781 on 20240123 end
    }
