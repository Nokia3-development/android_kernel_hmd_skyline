...
#ifdef CONFIG_ARCH_TOMCAT
#include <linux/thermal.h>

#define WLS_THERMAL_T0    46
#define WLS_THERMAL_T1    48
#define WLS_THERMAL_T2    50
#define WLS_THERMAL_T3    52
#define WLS_THERMAL_T4    54
#define DESCEND_TEMP      2
#define LCD_ON_OFFSET     5

typedef enum wls_thermal_sm {
	WLS_TEMP_BELOW_T0,
	WLS_TEMP_T0_to_T1,
	WLS_TEMP_T1_to_T2,
	WLS_TEMP_T2_to_T3,
...
u32 wls_icl_limit[] = {1250000, 1000000, 750000, 600000, 600000, 600000, 750000, 600000, 500000, 500000, 500000};

struct delayed_work *g_wls_thermal_work;
#endif
...
// Modify by liufurong for QN7026A-127 on 20240201 begin
#ifdef CONFIG_ARCH_TOMCAT
#define BC_GET_WLS_WORK_MODE		0x81
#endif
// Modify by liufurong for QN7026A-127 on 20240201 end
// Modify by jiashixian for TCT-835 on 202400301 begin
#ifdef CONFIG_ARCH_TOMCAT
static bool s_charge_lcd_screen_on = false;
#endif
// Modify by jiashixian for TCT-835 on 202400301 end
// Modify by liufurong for QN7026A-240 on 202400311 begin
// Modify by liufurong for QN7026A-426 on 202400328 begin
// Modify by liufurong for QN7026A-479/QN7026A-620/QN7026A-666 on 202400514 begin
u32 wire_lcd_off_fcc[] = {6000000, 6000000, 6000000, 5000000, 4000000, 3000000, 3000000, 3000000, 3000000, 3000000, 3000000};
#ifdef CONFIG_CUSTOM_FACTORY_BUILD
u32 wire_lcd_on_fcc[] = {6000000, 4000000, 3000000, 2500000, 2000000, 1500000, 1500000, 1500000, 1500000, 1500000, 1500000};
#else
u32 wire_lcd_on_fcc[] = {6000000, 3000000, 2000000, 1000000, 800000, 600000, 600000, 600000, 600000, 600000, 600000};
#endif
// Modify by liufurong for QN7026A-479/QN7026A-620/QN7026A-666 on 202400514 end
u32 wireless_lcd_off_icl[] = {1250000, 1250000, 1250000, 1250000, 1250000, 1250000, 1000000, 750000, 600000, 500000, 400000};
u32 wireless_lcd_on_icl[] = {1250000, 1250000, 1250000, 1250000, 1250000, 1250000, 750000, 600000, 500000, 400000, 200000};
// Modify by liufurong for QN7026A-426 on 202400328 end
// Modify by liufurong for QN7026A-240 on 202400311 end
...
struct battery_chg_dev {
	...
	// Modify by liufurong for QN7026A-538 on 20240413 begin
	#ifdef CONFIG_ARCH_TOMCAT
	struct delayed_work check_ntc_work;
	WLS_THERMAL_SM_T wls_sm;
	#endif
	// Modify by liufurong for QN7026A-538 on 20240413 end
	int				fake_soc;
	bool				block_tx;
...
// Modify by wenyaqi for TCT-2573 on 20240319 begin
#ifndef CONFIG_ARCH_TOMCAT
...
#endif
// Modify by wenyaqi for TCT-2573 on 20240319 end
				...
				size_t len)
{
	...
	// Modify by liufurong for QN7026A-127 on 20240201 begin
	#ifdef CONFIG_ARCH_TOMCAT
	struct wireless_get_mode_resp *work_mode_msg;
	#endif
	// Modify by liufurong for QN7026A-127 on 20240201 end
	struct psy_state *pst;
	bool ack_set = false;
	...
	// Modify by liufurong for QN7026A-127 on 20240201 begin
	#ifdef CONFIG_ARCH_TOMCAT
	case BC_GET_WLS_WORK_MODE:
		if (len == sizeof(*work_mode_msg)) {
			work_mode_msg = data;
			bcdev->wls_work_mode = work_mode_msg->work_mode;
			ack_set = true;
		} else {
			pr_err("Incorrect response length %zu for wls_get_work_mode\n",
				len);
		}
		break;
	#endif
...
// Modify by liufurong for QN7026A-240 on 202400311 begin
static int __battery_psy_set_wls_icl(struct battery_chg_dev *bcdev,
					u32 icl_ua)
{
	int rc;

	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_BATTERY],
				BATT_WLS_ICL_LIMT, icl_ua);
	if (rc < 0) {
		pr_err("Failed to set icl_ua %u, rc=%d\n", icl_ua, rc);
	} else {
		pr_debug("Set icl_ua to %u uA\n", icl_ua);
	}
...
{
	int rc;
	/*Modify by jiashixian  for TCT-835 on 20240301 begin*/
	#ifdef CONFIG_ARCH_TOMCAT
	/*Modify by liufurong for QN7026A-524 on 20240410 begin*/
	pr_err("start the s_screen bright = %d, thermal_level = %d\n", s_charge_lcd_screen_on, val);
	if (val > 10) {
		return 0;
	}
	/*Modify by liufurong for QN7026A-524 on 20240410 end*/
	if (val == 0) { //clean thermal limit
		rc = __battery_psy_set_wls_icl(bcdev, wireless_lcd_on_icl[val]);
		if (rc) {
			pr_err("%s set wls icl fail val = %d\n", __func__, val);
		}
		rc = __battery_psy_set_charge_current(bcdev, wire_lcd_on_fcc[val]);
		if (rc) {
			pr_err("%s set wire icl fail val = %d\n", __func__, val);
		} else {
			bcdev->thermal_fcc_ua = wire_lcd_on_fcc[val];
		}
	} else if (s_charge_lcd_screen_on == true) { // lcd on
		if (val <= 5) { //wire limit
			rc = __battery_psy_set_charge_current(bcdev, wire_lcd_on_fcc[val]);
			if (rc) {
				pr_err("%s set wire icl fail val = %d\n", __func__, val);
		...
		if (val <= 5) { // wire limit
			rc = __battery_psy_set_charge_current(bcdev, wire_lcd_off_fcc[val]);
			if (rc) {
				pr_err("%s set wire icl fail val = %d\n", __func__, val);
			} else {
				bcdev->thermal_fcc_ua = wireless_lcd_off_icl[val];
			}
		} else { // wireless limit
			rc = __battery_psy_set_wls_icl(bcdev, wireless_lcd_off_icl[val]);
			if (rc) {
				pr_err("%s set wls icl fail val = %d\n", __func__, val);
			}
		}
	...
	pr_err("the s_screen bright = %d, thermal_level = %d\n", s_charge_lcd_screen_on, val);

	return rc;
	#else
	/*Modify by jiashixian  for TCT835 on 20240301 end*/
...
static enum power_supply_property battery_props[] = {
	...
	// Modify by wenyaqi for TCT-2746 on 20240321 begin
	#ifndef CONFIG_ARCH_TOMCAT
	POWER_SUPPLY_PROP_TIME_TO_FULL_AVG,
	POWER_SUPPLY_PROP_TIME_TO_FULL_NOW,
	POWER_SUPPLY_PROP_TIME_TO_EMPTY_AVG,
	#endif
	// Modify by wenyaqi for TCT-2746 on 20240321 end
...
static int wireless_fw_update(struct battery_chg_dev *bcdev, bool force)
{
	...
	if (!pst->prop[USB_ONLINE]) {
		...
		// Modify by liufurong for QN7026A-127 on 20240201 begin
		#ifdef CONFIG_ARCH_TOMCAT
		if ((pst->prop[BATT_CAPACITY] / 100) < 10) {
			pr_err("Battery SOC should be at least 10%% or connect charger\n");
			rc = -EINVAL;
			goto out;
		}
		#else
...
// Modify by liufurong for QN7026A-127 on 20240201 begin
#ifdef CONFIG_ARCH_TOMCAT
static ssize_t wireless_work_mode_show(struct class *c,
					struct class_attribute *attr,
					char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
	struct wireless_get_mode_resp req_msg = {};
	int rc;

	req_msg.hdr.owner = MSG_OWNER_BC;
	req_msg.hdr.type = MSG_TYPE_REQ_RESP;
	req_msg.hdr.opcode = BC_GET_WLS_WORK_MODE;

	rc = battery_chg_write(bcdev, &req_msg, sizeof(req_msg));
	if (rc < 0) {
		pr_err("Failed to get work mode rc=%d\n", rc);
		return rc;
	}

	return scnprintf(buf, PAGE_SIZE, "%#x\n", bcdev->wls_work_mode);
}
static CLASS_ATTR_RO(wireless_work_mode);
#endif
// Modify by liufurong for QN7026A-127 on 20240201 end
...
static struct attribute *battery_class_attrs[] = {
	...
	// Modify by liufurong for QN7026A-127 on 20240201 begin
	#ifdef CONFIG_ARCH_TOMCAT
	&class_attr_wireless_work_mode.attr,
	#endif
	// Modify by liufurong for QN7026A-127 on 20240201 end
	NULL,
};
...
// Modify by jiashixian for TCT-835 on 20240301 begin
		#ifdef CONFIG_ARCH_TOMCAT
		s_charge_lcd_screen_on = false;
		#endif
// Modify by jiashixian for TCT-835 on 20240301 end
// Modify by wenyaqi for TCT-2573 on 20240319 begin
		#ifndef CONFIG_ARCH_TOMCAT
...
// Modify by liufurong for QN7026A-538 on 20240413 begin
#ifdef CONFIG_ARCH_TOMCAT
void schedule_wls_thermal_work(bool enable)
{
	pr_err("schedule_wls_thermal_work enable = %d\n", enable);
	if (enable) {
		schedule_delayed_work(g_wls_thermal_work, msecs_to_jiffies(100));
	} else {
		cancel_delayed_work(g_wls_thermal_work);
	}
}
EXPORT_SYMBOL(schedule_wls_thermal_work);

static void check_ntc_temp_work(struct work_struct *work)
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, check_ntc_work.work);
	// Modify by liufurong for TCT-5060 on 20240509 begin
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_BATTERY];
	// Modify by liufurong for TCT-5060 on 20240509 end

	struct thermal_zone_device *tz = NULL;
	int sys_therm_temp = 0;
	int ret = 0;
	WLS_THERMAL_SM_T pre_sm = bcdev->wls_sm;

	tz = thermal_zone_get_zone_by_name("sys-therm-1");
	if (IS_ERR(tz)) {
		return;
	}
	ret = thermal_zone_get_temp(tz, &sys_therm_temp);
	if (ret != 0) {
		return;
	}
	sys_therm_temp /= 1000;

	if (sys_therm_temp >= WLS_THERMAL_T4) {
		bcdev->wls_sm = WLS_TEMP_ABOVE_T4;
	} else if (sys_therm_temp >= WLS_THERMAL_T3) {
		if (bcdev->wls_sm == WLS_TEMP_ABOVE_T4 && sys_therm_temp > WLS_THERMAL_T4 - DESCEND_TEMP) {
			bcdev->wls_sm = WLS_TEMP_ABOVE_T4;
		} else {
			bcdev->wls_sm = WLS_TEMP_T3_to_T4;
		}
	} else if (sys_therm_temp >= WLS_THERMAL_T2) {
		if (bcdev->wls_sm == WLS_TEMP_T3_to_T4 && sys_therm_temp > WLS_THERMAL_T3 - DESCEND_TEMP) {
			bcdev->wls_sm = WLS_TEMP_T3_to_T4;
		} else {
			bcdev->wls_sm = WLS_TEMP_T2_to_T3;
		}
	} else if (sys_therm_temp >= WLS_THERMAL_T1) {
		if (bcdev->wls_sm == WLS_TEMP_T2_to_T3 && sys_therm_temp > WLS_THERMAL_T2 - DESCEND_TEMP) {
			bcdev->wls_sm = WLS_TEMP_T2_to_T3;
		} else {
			bcdev->wls_sm = WLS_TEMP_T1_to_T2;
		}
	} else if (sys_therm_temp >= WLS_THERMAL_T0) {
		if (bcdev->wls_sm == WLS_TEMP_T1_to_T2 && sys_therm_temp > WLS_THERMAL_T1 - DESCEND_TEMP) {
			bcdev->wls_sm = WLS_TEMP_T1_to_T2;
		} else {
			bcdev->wls_sm = WLS_TEMP_T0_to_T1;
		}
	} else {
		if (bcdev->wls_sm == WLS_TEMP_T0_to_T1 && sys_therm_temp > WLS_THERMAL_T0 - DESCEND_TEMP) {
			bcdev->wls_sm = WLS_TEMP_T0_to_T1;
		} else {
			bcdev->wls_sm = WLS_TEMP_BELOW_T0;
		}
	}

	if (pre_sm != bcdev->wls_sm) {
		if (s_charge_lcd_screen_on == true && bcdev->wls_sm != WLS_TEMP_BELOW_T0) {
			__battery_psy_set_wls_icl(bcdev, wls_icl_limit[bcdev->wls_sm + LCD_ON_OFFSET]);
		} else {
			__battery_psy_set_wls_icl(bcdev, wls_icl_limit[bcdev->wls_sm]);
		}
	}
	// Modify by liufurong for TCT-5060 on 20240509 begin
	power_supply_changed(pst->psy);
	pr_err("check_ntc_temp_work add sys thermal = %d, bcdev->wls_sm = %d\n", sys_therm_temp, bcdev->wls_sm);
	// Modify by liufurong for TCT-5060 on 20240509 end

	schedule_delayed_work(g_wls_thermal_work, msecs_to_jiffies(1000));
}
#endif
// Modify by liufurong for QN7026A-538 on 20240413 end
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	// Modify by liufurong for QN7026A-538 on 20240413 begin
	#ifdef CONFIG_ARCH_TOMCAT
	INIT_DELAYED_WORK(&bcdev->check_ntc_work, check_ntc_temp_work);
	g_wls_thermal_work =  &bcdev->check_ntc_work;
	#endif
	// Modify by liufurong for QN7026A-538 on 20240413 end
	bcdev->dev = dev;
