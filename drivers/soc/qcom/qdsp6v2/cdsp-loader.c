...
// [TCT-319]Add by zhixue.chang for reboot to red screen if check modem is not signed 20240227 begin
#include <linux/reboot.h>
// [TCT-319]Add by zhixue.chang for reboot to red screen if check modem is not signed 20240227 end

// modify by zhixue.chang for get fuse and bootloader state 20240307 begin
/**
...
// modify by zhixue.chang for get fuse and bootloader state 20240307 end
...
fail:
	if (pdev)
		// modify by zhixue.chang for get fuse and bootloader state 20240307 begin
		dev_err(&pdev->dev, "%s : get_efuse_status: %d\n",
					__func__, get_efuse_status());
		dev_err(&pdev->dev, "%s : is_bootloader_unlock: %d\n",
					__func__, is_bootloader_unlock());
		// modify by zhixue.chang for get fuse and bootloader state 20240307 end
		dev_err(&pdev->dev,
			"%s: CDSP image loading failed\n", __func__);
		// [TCT-319]Add by zhixue.chang for reboot to red screen if check modem is not signed 20240227 begin
		if ((true == get_efuse_status()) && (false == is_bootloader_unlock())) {
			dev_err(&pdev->dev, "CDSP image is unsigned or corrupt in the efuse device, now reboot to red screen!\n");
