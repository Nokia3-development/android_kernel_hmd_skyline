...
/* Modify by yubo for QN7026A-54 on 20240305 begin */
#define DISCARD_FRAMES						3
/* Modify by yubo for QN7026A-54 on 20240305 end */
...
{
	int ret = 0;
	char save_path[FILE_NAME_LENGTH] = {0};
	struct file *fp = NULL;
	char file_name[FILE_NAME_LENGTH] = {0};

	/* format result file */
	goodix_strncat(file_name, GOODIX_RESULT_SAVE_PATH, FILE_NAME_LENGTH);
	goodix_strncat(file_name, (g_core_data->result == 0) ? "FAIL_" : "PASS_", FILE_NAME_LENGTH);
	goodix_strncat(file_name, get_date_time_str(), FILE_NAME_LENGTH);
	goodix_strncat(file_name, "_GT9916R_BOE", FILE_NAME_LENGTH);
	goodix_strncat(file_name, ".csv", FILE_NAME_LENGTH);
	scnprintf(save_path, ARRAY_SIZE(save_path), file_name);

	fp = filp_open(save_path, O_CREAT | O_RDWR | O_TRUNC, 0666);
	if (IS_ERR(fp)) {
...
static struct platform_device goodix_info_device = {
	.name = GOODIX_INFO_NAME,
	.id = -1,
};

static ssize_t goodix_ito_test_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	int ret = 0;
	struct ts_rawdata_info *info = NULL;

	ts_info("ito test start");
	if (g_core_data == NULL) {
		return -EFAULT;
	}
	...
	goodix_do_inspect(g_core_data, info);
	ret = sprintf(buf, "%s\n", (g_core_data->result == 0) ? "0" : "1");
	ts_info("test_result:%s", info->result);

	kfree(info);
	ts_info("ito test end");
	return ret;
}

static ssize_t goodix_ito_test_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	return count;
}

static DEVICE_ATTR(factory_check, 0644, goodix_ito_test_show, goodix_ito_test_store);
static struct attribute *goodix_ito_test_atts[] = {
		&dev_attr_factory_check.attr,
		NULL,
};

static const struct attribute_group goodix_ito_test_attribute_group = {
	.attrs = goodix_ito_test_atts,
};
...
int ito_test_node_init(struct platform_device *tp_device)
{
	...
	ret = sysfs_create_group(&tp_device->dev.kobj, &goodix_ito_test_attribute_group);
	if (ret != 0) {
		sysfs_remove_group(&tp_device->dev.kobj, &goodix_ito_test_attribute_group);
		return ret;
	}
	return 0;
}

void ito_test_node_exit(struct platform_device *tp_device)
{
	sysfs_remove_group(&tp_device->dev.kobj, &goodix_ito_test_attribute_group);
	platform_device_unregister(tp_device);
}
...
int inspect_module_init(void)
{
	...
	/* create sysfs */
	ret = ito_test_node_init(&goodix_info_device);
	if (ret) {
		ts_err("ito_test_node_init failed\n");
		goto err_out;
	}
...
void inspect_module_exit(void)
{
	...
	ito_test_node_exit(&goodix_info_device);
