...
// Modify by liuqingxuan for TCT-2716  add GPIO dump info when system suspend 20240318 begin
#include <trace/events/power.h>
#include <linux/fcntl.h>
#include <linux/seq_file.h>
#include <linux/printk.h>
// Modify by liuqingxuan for TCT-2716  add GPIO dump info when system suspend 20240318 end
...
// Modify by liuqingxuan for TCT-2716  add GPIO dump info when system suspend 20240318 end
{
	...
	if (gpio_info){
		int pulls_keeper_hq[] = {3,0,2,1};
		int pulls_no_keeper_hq[] = {3,0,1,};
		gpio_info->is_out = is_out;
		gpio_info->value = val;
		gpio_info->func = func;
		gpio_info->mA = msm_regval_to_drive(drive);
		if (pctrl->soc->pull_no_keeper){
			gpio_info->pull = pulls_no_keeper_hq[pull];
		}
		else {
			gpio_info->pull = pulls_keeper_hq[pull];
		}
	}
	// Modify by liuqingxuan for TCT-2716  add GPIO dump info when system suspend 20240318 end
}
// Modify by liuqingxuan for TCT-2716  add GPIO dump info when system suspend 20240318 begin
unsigned short generateGPIOStatus(int direction, int value, int pull, int function, int power){
    /*
     * direction 1 bit,
     * value: 1 bit
     * pull: 3 bit
     ...
     */
    int output = 0;
    output |= (direction & 0x1) << 15;
    output |= (value & 0x1) <<14;
    output |= (pull & 0x7) << 11;
    output |= (function & 0x1F) << 6;
    output |= (power & 0x3F);
    return output;
}
...
static void msm_gpio_dbg_show(struct seq_file *s, struct gpio_chip *chip)
{
	...
	for (i = 0; i < chip->ngpio; i++, gpio++){
		struct gpio_info gpio_info;
		msm_gpio_dbg_show_one(s, NULL, chip, i, gpio, &gpio_info);
		flags = generateGPIOStatus(gpio_info.is_out,gpio_info.value, gpio_info.pull,gpio_info.func,gpio_info.mA);
		sprintf(gpioall+(i * 4), "%04X", flags);
	}
	printk("%s", gpioall);
}
static void gpio_dump_suspend_trace(void *unused,const char *action, int val, bool start)
{
		      ...
		      const struct msm_pinctrl_soc_data *soc_data)
{
	...
	// Modify by liuqingxuan for TCT-2716  add GPIO dump info when system suspend 20240318 begin
	ret = register_trace_suspend_resume(gpio_dump_suspend_trace, NULL);
	if (ret)
		pr_info("register_trace_suspend_resume 001 failed.\n");
	// Modify by liuqingxuan for TCT-2716  add GPIO dump info when system suspend 20240318 end
	return 0;
}
...
int msm_pinctrl_remove(struct platform_device *pdev)
{
	...
	// Modify by liuqingxuan for TCT-2716  add GPIO dump info when system suspend 20240318 begin
	unregister_trace_suspend_resume(gpio_dump_suspend_trace, NULL);
	// Modify by liuqingxuan for TCT-2716  add GPIO dump info when system suspend 20240318 end
	return 0;
}
