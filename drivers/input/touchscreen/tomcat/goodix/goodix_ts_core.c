...
/* Modify by liudi for TCT-787 on 20240110 begin */
#include <linux/soc/qcom/panel_event_notifier.h>
/* Modify by liudi for TCT-787 on 20240110 end */
...
/* Modify by tangzhen for TCT-998 on 20240118 begin */
#ifdef CONFIG_CUSTOM_FACTORY_BUILD
static bool g_finger_flag[GOODIX_MAX_TOUCH] = {false};
#endif
/* Modify by tangzhen for TCT-998 on 20240118 end */
/* goodix fb test */
...
static void goodix_register_for_panel_events(struct device_node *dp, struct goodix_ts_core *cd)
{
	void *cookie = NULL;
...
/* Modify by tangzhen for TCT-998 on 20240118 begin */
#ifdef CONFIG_CUSTOM_FACTORY_BUILD
static bool point_all_up(void)
{
	int i;

	for (i = 0; i < GOODIX_MAX_TOUCH; i++) {
		if (g_finger_flag[i] == true) {
			return false;
		}
	}
		...
		struct goodix_touch_data *touch_data)
{
	...
	for (i = 0; i < GOODIX_MAX_TOUCH; i++) {
		if (touch_data->coords[i].status == TS_TOUCH) {
			/* Modify by tangzhen for TCT-998 on 20240118 begin */
			#ifdef CONFIG_CUSTOM_FACTORY_BUILD
			if (g_finger_flag[i] == false) {
				ts_info("P[%d] down, x %d, y %d, w %d", i,
					touch_data->coords[i].x,
					touch_data->coords[i].y,
					touch_data->coords[i].w);
				g_finger_flag[i] = true;
			}
			#endif
			/* Modify by tangzhen for TCT-998 on 20240118 end */
			...
			if (g_finger_flag[i] == true) {
				ts_info("P[%d] UP", i);
				g_finger_flag[i] = false;
				if (point_all_up()) {
					ts_info("Points ALL Up");
				}
			}
			#endif
			/* Modify by tangzhen for TCT-998 on 20240118 end */
 ...
 * return: 0 ok, <0 failed
 */
/* Modify by yubo for TCT-6068 on 20240527 begin */
static irqreturn_t goodix_ts_threadirq_func(int irq, void *data)
{
	...
	#if defined(CONFIG_PM) && IS_ENABLED(GOODIX_PATCH_COMERR_PM)
	if ((atomic_read(&core_data->suspended)) && (core_data->pm_suspend)) {
		ret = wait_for_completion_timeout(
				&core_data->pm_completion,
				msecs_to_jiffies(GOODIX_TIMEOUT_COMERR_PM));
		if (!ret) {
...
/* Modify by yubo for TCT-6068 on 20240527 end */
...
static int goodix_ts_input_dev_config(struct goodix_ts_core *core_data)
{
	...
	/* Modify by liudi for TCT-648 on 20240126 begin */
	set_bit(SINGLE_WAKEUP, input_dev->keybit);
	/* Modify by liudi for TCT-648 on 20240126 end */
	...
	struct panel_event_notification *notification, void *client_data)
{
	...
	ts_info("Notification type:%d, early_trigger:%d",notification->notif_type,
	...
	switch (notification->notif_type) {
		...
		/* Modify by liudi for TCT-999 on 20240120 begin */
		...
		case DRM_PANEL_EVENT_FPS_CHANGE:
			ts_info("Received fps change old fps:%d new fps:%d\n", notification->notif_data.old_fps,
...
static int goodix_ts_pm_suspend(struct device *dev)
{
	...
	ts_debug("system enters into pm_suspend");
...
static int goodix_ts_pm_resume(struct device *dev)
{
	...
	ts_debug("system resumes from pm_suspend");
...
/* Modify by liudi for TCT-787 on 20240110 end */
	...
	/* inspect init */
	inspect_module_init();
	/* Modify by liudi for TCT-999 on 20240120 end */
...
/* Modify by tangzhen for TCT-837 on 20240514 begin */
bool tp_get_bootargs(const char *info)
{
	const char *bootparams = NULL;
	bool found = false;
	struct device_node *np = of_find_node_by_path("/chosen");
	if (!np) {
		ts_err("%s: failed to find node", __func__);
		return false;
	}
	if (of_property_read_string(np, "bootargs", &bootparams)) {
		ts_err("%s: failed to get bootargs property", __func__);
		of_node_put(np);
		return false;
	}
	found = strnstr(bootparams, info, strlen(bootparams)) ? true : false;
	if (found) {
		ts_info("%s: %s", __func__, info);
	}
	of_node_put(np);
	return found;
}

static ssize_t lcd_info_show(struct kobject *kobj,
			struct kobj_attribute *attr, char *buf)
{
	if (tp_get_bootargs("nt37701a_boe")) {
...
static ssize_t tp_info_show(struct kobject *kobj,
			struct kobj_attribute *attr, char *buf)
{
	char tp_info[PAGE_SIZE] = {0};
	if (g_core_data == NULL) {
		return -EFAULT;
 	}

	snprintf(tp_info, sizeof(tp_info), "Module_name:GT9916R_BOE TOUCH_VER:%02x cfg_version:%01X", g_core_data->fw_version.patch_vid[3] & 0xFF, g_core_data->ic_info.version.config_version);

	return scnprintf(buf, PAGE_SIZE, "%s\n", tp_info);
}

static ssize_t fp_info_show(struct kobject *kobj,
			struct kobj_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s\n", FP_ID);
}

static struct kobj_attribute tp_info_attr = __ATTR(tp_info, 0444, tp_info_show, NULL);
static struct kobj_attribute lcd_info_attr = __ATTR(lcd_info, 0444, lcd_info_show, NULL);
static struct kobj_attribute fp_info_attr = __ATTR(fp_info, 0444, fp_info_show, NULL);

static struct attribute *lcd_tp_fp_info_attrs[] = {
	&tp_info_attr.attr,
	&lcd_info_attr.attr,
	&fp_info_attr.attr,
	NULL,
...
static const struct attribute_group lcd_tp_fp_info_group = {
	.attrs = lcd_tp_fp_info_attrs,
};
static const struct attribute_group *attr_groups[] = {
	&lcd_tp_fp_info_group,
	NULL,
};
/* Modify by tangzhen for TCT-837 on 20240514 end */

/* Modify by liudi for TCT-1000 on 20240112 begin */
static struct proc_dir_entry *goodix_info_proc_entry = NULL;
struct goodix_ts_core *g_core_data = NULL;
/* Modify by liudi for TCT-1003 on 20240226 begin */
static ssize_t goodix_proc_getinfo_read(struct file *flip,char __user *buff,size_t size,loff_t *pPos)
{
    ...
    snprintf(buf, sizeof(buf), "Module_name:GT9916R_BOE TOUCH_VER:%02x cfg_version:%01X\n", g_core_data->fw_version.patch_vid[3] & 0xFF, g_core_data->ic_info.version.config_version);
    rc = simple_read_from_buffer(buff,size,pPos,buf,strlen(buf));
    return rc;
}
/* Modify by liudi for TCT-1003 on 20240226 end */
static const struct proc_ops goodix_info_proc_fops = {
    .proc_read = goodix_proc_getinfo_read,
};
static int32_t goodix_extra_proc_init(void)
{
    goodix_info_proc_entry = proc_create(GOODIX_INFO_PROC_FILE,0666,NULL,&goodix_info_proc_fops);
    if (NULL == goodix_info_proc_entry) {
        ts_err("Couldn't create proc entry!");
        return -EFAULT;
    } else {
        ts_info("Create proc entry success");
    }
    return 0;
}
/* Modify by liudi for TCT-1000 on 20240112 end */

/* Modify by liudi for TCT-648 on 20240126 begin */
/* Modify by liudi for QN7026A-22 on 20240118 begin */
static ssize_t show_double_en(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	type = g_core_data->gesture_type;

	return sprintf(buf, "%s\n",
			(type & GESTURE_DOUBLE_TAP) ? "enable" : "disable");
}

static ssize_t store_double_en(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '1') {
		...
		g_core_data->gesture_type |= GESTURE_DOUBLE_TAP;
	} else if (buf[0] == '0') {
...
static struct platform_device goodix_gesture_device = {
	.name = "tp_gesture",
	.id = -1,
};
static DEVICE_ATTR(double_en, 0644, show_double_en, store_double_en);

static ssize_t show_single_en(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned char type = {0};
...
static ssize_t store_single_en(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '1') {
		...
		g_core_data->gesture_type |= GESTURE_SINGLE_TAP;
	} else if (buf[0] == '0') {
...
static DEVICE_ATTR(single_en, 0644, show_single_en, store_single_en);

int gesture_node_init(struct platform_device *tp_device)
{
	int ret = 0;
	ret = platform_device_register(tp_device);
	if (ret != 0) {
		ts_err("platform_device_register fail");
		platform_device_unregister(tp_device);
		return ret;
	}

	ret = device_create_file(&tp_device->dev, &dev_attr_double_en);
	if (ret != 0) {
		ts_err("double_device_create_file fail");
		device_remove_file(&tp_device->dev, &dev_attr_double_en);
		return ret;
	}

	ret = device_create_file(&tp_device->dev, &dev_attr_single_en);
	if (ret != 0) {
		ts_err("single_device_create_file fail");
		device_remove_file(&tp_device->dev, &dev_attr_single_en);
		return ret;
	}
	return 0;
}

void gesture_node_exit(struct platform_device *tp_device)
{
	device_remove_file(&tp_device->dev, &dev_attr_single_en);
	device_remove_file(&tp_device->dev, &dev_attr_double_en);
	platform_device_unregister(tp_device);
}
/* Modify by liudi for QN7026A-22 on 20240118 end */
/* Modify by liudi for TCT-648 on 20240126 end */
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	if (core_data != NULL) {
		...
		goodix_extra_proc_init();
	}
	/* Modify by liudi for TCT-1000 on 20240112 end */
	/* Modify by liudi for QN7026A-22 on 20240118 begin */
	ret = gesture_node_init(&goodix_gesture_device);
	if (ret) {
		ts_err("gesture_node_init failed\n");
		goto err_out;
	}
	/* Modify by liudi for QN7026A-22 on 20240118 end */
...
static int goodix_ts_remove(struct platform_device *pdev)
{
	...
	/* Modify by liudi for QN7026A-22 on 20240118 begin */
	gesture_node_exit(&goodix_gesture_device);
	/* Modify by liudi for QN7026A-22 on 20240118 end */
...
/* Modify by liudi for TCT-1005 on 20240131 begin */
bool tp_get_boot_mode(void) {
	...
	if (!touchload) {
		ts_err("it is not normal boot!");
	} else if (strnstr(touchload, "touch.load=1", strlen(touchload))) {
		ts_info("it is normal boot!");
		return true;
	}
	return false;
}
/* Modify by liudi for TCT-1005 on 20240131 end */
...
static int __init goodix_ts_core_init(void)
{
	...
	if (ret < 0) {
		ts_err("%s: sysfs_create_groups fail ret = %d\n", __func__, ret);
	}
	/* Modify by tangzhen for TCT-837 on 20240514 end */
