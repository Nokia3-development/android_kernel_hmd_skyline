		...
		struct goodix_ext_module *module)
{
	...
	/* Modify by liudi for TCT-1002 on 20240123 begin */
	...
	#endif
	/* Modify by liudi for TCT-1002 on 20240123 end */
	...
	struct goodix_ext_module *module)
{
	...
	case GOODIX_GESTURE_SINGLE_TAP:
		if (cd->gesture_type & GESTURE_SINGLE_TAP) {
			...
			input_report_key(cd->input_dev, SINGLE_WAKEUP, 1);
			// input_report_key(cd->input_dev, KEY_GOTO, 1);
			input_sync(cd->input_dev);
			input_report_key(cd->input_dev, SINGLE_WAKEUP, 0);
