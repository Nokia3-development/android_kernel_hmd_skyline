...
/* Added by huofudong for TCT-987 on 20240115 begin */
#ifdef SX933X_FACTORY_NODE
#include "sensor_fac_node.h"
#endif
/* Added by huofudong for TCT-987 on 20240115 end */
/* Added by huofudong for QN7026A-29 on 20240123 begin */
#include "sensor_user_node.h"
static bool g_onoff_enable = false;
/* Added by huofudong for QN7026A-29 on 20240123 end */

#define USE_THREAD_IRQ              1
#define ENABLE_STAY_AWAKE           0
/* Modify by huofudong for QN7026A-280 on 20240321 begin */
#define ENABLE_ESD_RECOVERY         1
/* Modify by huofudong for QN7026A-280 on 20240321 end */
...
/* Added by huofudong for TCT-987 | QN7026A-29 on 20240123 begin */
...
/* Added by huofudong for TCT-987 | QN7026A-29 on 20240123 end */

/* Added by huofudong for QN7026A-29 | QN7026A-63  | QN7026A-156 on 20240226 begin */
struct dumpinfo {
    int ph_state;
    u16 ref_cap;
    u16 total_cap;
...
/* Modify by huofudong for QN7026A-689 on 20240530 begin */
#ifdef SX933X_ANFR_LOGIC
const int g_irq_maxnum = 50;
...
#endif
/* Modify by huofudong for QN7026A-689 on 20240530 end */
struct raw_data_t{
    s32 useful;
    s32 average;
    s32 diff;
...
struct raw_data_t raw_data_phase[6];

static int gs_up_p1 = 0;
static int gs_up_p2 = 0;
static int gs_down_p1 = 0;
static int gs_down_p2 = 0;
/* Added by huofudong for QN7026A-192 | QN7026A-682 on 20240522 begin */
static bool g_Enabled = false;
static bool g_sar_Invalid = false;
static bool g_flag = true;
/* Added by huofudong for QN7026A-192 | QN7026A-682 on 20240522 end */
/* Added by huofudong for QN7026A-29 | QN7026A-63  | QN7026A-156 on 20240226 end */
...
/* Modify by huofudong for QN7026A-682 on 20240522 begin */
    int irq_disabled;
...
/* Modify by huofudong for QN7026A-682 on 20240522 end */
//#################################################################################################
/* Added by huofudong for TCT-987 QN7026A-29 on 20240123 begin */
...
/* Added by huofudong for QN7026A-29 on 20240123 end */
	...
	rtc_ktime_to_tm(txc.tv_sec);

#if EXTRA_LOG_DATE_TIME
    sprintf(log_time, "%02d-%02d %02d:%02d:%02d.%03d", 
        tm.tm_mon, tm.tm_mday,tm.tm_hour,tm.tm_min,tm.tm_sec, (int)txc.tv_sec/1000);
#else
	sprintf(log_time, "%02d:%02d:%02d.%03d", 
        tm.tm_hour, tm.tm_min, tm.tm_sec, (int)txc.tv_sec/1000);
#endif

    return log_time;
}
/* Modify by huofudong for TCT-987 | QN7026A-29 on 20240123 begin */
#define SMTC_LOG_DBG(fmt, args...)   pr_info(" SENS_SAR:%s D "  LOG_CHIP_NAME "%s(%d):" fmt "\n", smtc_get_log_time(), __func__, __LINE__, ##args)
#define SMTC_LOG_INF(fmt, args...)   pr_info (" SENS_SAR:%s I "  LOG_CHIP_NAME "%s(%d):" fmt "\n", smtc_get_log_time(), __func__, __LINE__, ##args)
#define SMTC_LOG_WRN(fmt, args...)   pr_warn (" SENS_SAR:%s W "  LOG_CHIP_NAME "%s(%d):" fmt "\n", smtc_get_log_time(), __func__, __LINE__, ##args)
#define SMTC_LOG_ERR(fmt, args...)   pr_err  (" SENS_SAR:%s E "  LOG_CHIP_NAME "%s(%d):" fmt "\n", smtc_get_log_time(), __func__, __LINE__, ##args)
/* Modify by huofudong for TCT-987 | QN7026A-29 on 20240123 end */
...
static void smtc_report_force_detected(Self self)
{
    ...
    phase_p phase;
    struct input_dev *input;
    SMTC_LOG_WRN("Report force detected");
    for (ph = 0; ph < NUM_PHASES; ph++) {
        ...
        SMTC_LOG_INF("%s smtc_report_force_detected", phase->name);
    }
}
/* Modify by huofudong for QN7026A-682 on 20240522 end */
//#################################################################################################
...
/* Added by huofudong for QN7026A-156 on 20240226 begin */
static ssize_t sx933x_raw_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    u32 uData;
    u16 offset;
    Self this = dev_get_drvdata(dev);
    if(this) {
        for(csx =0; csx<6; csx++) {
            smtc_i2c_read(this, SX933X_USEPH0_REG + csx*4, &uData);
            useful = (s32)uData>>10;
            smtc_i2c_read(this, SX933X_AVGPH0_REG + csx*4, &uData);
            average = (s32)uData>>10;
            smtc_i2c_read(this, SX933X_DIFFPH0_REG + csx*4, &uData);
            diff = (s32)uData>>10;
            smtc_i2c_read(this, SX933X_OFFSETPH0_REG + csx*8, &uData);
            offset = (u16)(uData & 0x7FFF);
            raw_data_phase[csx].useful = useful;
            raw_data_phase[csx].average = average;
            raw_data_phase[csx].diff = diff;
            raw_data_phase[csx].offset = offset;
        }
        p += snprintf(p,PAGE_SIZE, "0,%d,%d,%d,%d,1,%d,%d,%d,%d,2,%d,%d,%d,%d,3,%d,%d,%d,%d,4,%d,%d,%d,%d,5,%d,%d,%d,%d,6 \n",
            ...
            raw_data_phase[4].useful,
            raw_data_phase[4].average,
            raw_data_phase[4].diff,
            raw_data_phase[4].offset,

            raw_data_phase[5].useful,
            raw_data_phase[5].average,
            raw_data_phase[5].diff,
            raw_data_phase[5].offset);
    }
    return (p-buf);
}

static ssize_t sx933x_regproxstatus_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    char *p = buf;
    u32 prox_state;
    ...
    if(this) {
        smtc_i2c_read(this, REG_PROX_STATUS, &prox_state);
        gs_up_p1 = (prox_state >> 26) & 1;
        gs_up_p2 = (prox_state >> 10) & 1;
        gs_down_p1 = (prox_state >> 24) & 1;
        gs_down_p2 = (prox_state >> 8) & 1;
        if(gs_up_p1 == 1 && gs_up_p2 == 1 && gs_down_p1 == 1 && gs_down_p2 == 0) {
            prox_state = prox_state & 0xEFFFEFF;
        }
        p += sprintf(p, "0x%x,0, \n", prox_state);
    }
    return (p-buf);
}
/* Added by huofudong for QN7026A-156 on 20240226 end */
...
/* Added by huofudong for QN7026A-71 on 20240206 begin */
//=================================================================================================
//EXIT ANFR
...
/* Added by huofudong for QN7026A-71 on 20240206 end */

//=================================================================================================
/* Added by huofudong for QN7026A-71 | QN7026A-156 on 20240226 begin */
...
static DEVICE_ATTR(raw_datad,      0444, smtc_raw_data_show,       NULL);
...
static DEVICE_ATTR(raw_data,       0444, sx933x_raw_data_show,     NULL);
static DEVICE_ATTR(regproxstatus,  0444, sx933x_regproxstatus_show,NULL);
#ifdef SX933X_ANFR_LOGIC
...
static struct attribute *smtc_sysfs_nodes_attr[] =
{
    ...
    &dev_attr_raw_datad.attr,
    &dev_attr_regproxstatus.attr,
    #ifdef SX933X_ANFR_LOGIC
...
/* Added by huofudong for QN7026A-71 | QN7026A-156 on 20240226 end */
//#################################################################################################
...
static int smtc_parse_dts(Self self)
{
    ...
    SMTC_LOG_INF("irq_gpio= %d", self->gpio);
        ...
        if(phen & 1<<ph)
        {
            ...
            for(idx=1; idx<_ESD_FAIL_CHECK_TIMES; idx++)
            {
                if(ph_useful[ph][idx-1] == ph_useful[ph][idx])
                {
                    ...
                    {
                        /* Added by huofudong for TCT-371 on 20240515 begin */
                        SMTC_LOG_ERR("Checking ESD failure");
                        /* Added by huofudong for TCT-371 on 20240515 end */
...
/* Added by huofudong for TCT-987 | QN7026A-29 | QN7026A-59 on 20240202 begin */
//=================================================================================================
//factory
#ifdef SX933X_FACTORY_NODE
...
#endif
/* Added by huofudong for TCT-987 | QN7026A-29 | QN7026A-59 on 20240202 end */

/* Added by huofudong for QN7026A-63 | QN7026A-689 on 20240530 begin */
//=================================================================================================
//ANFR
...
#endif
/* Added by huofudong for QN7026A-63 | QN7026A-689 on 20240530 end */

/* Added by huofudong for QN7026A-29 | QN7026A-59 on 20240202 begin */
//=================================================================================================
//user
    ...
    if (g_self != NULL) {
        if (sscanf(buf, "%d,%d", &phase, &enable) == 2)
        {
            ...
            if (enable == 1) {
                ...
                /* Added by huofudong for QN7026A-192 on 20240308 begin */
                if (g_self->channel_status & g_all_mask) {
                    g_Enabled = true;
                }
                /* Added by huofudong for QN7026A-192 on 20240308 end */
                ...
                /* Added by huofudong for QN7026A-63 | QN7026A-682 on 20240522 begin */
                if(g_sar_Invalid == true) {
                    smtc_report_force_detected(g_self);
                } else {
                    #ifdef SX933X_ANFR_LOGIC
                    if (g_anfr_flag) {
                ...
                /* Added by huofudong for QN7026A-63 | QN7026A-682 on 20240522 end */
            } else if (enable == 0) {
                if (0 == phase) {
                    strcpy(band_name, "SMTC SAR Sensor CH0");
                    if (g_self->channel_status & g_ch1_mask) {
...
/* Added by huofudong for QN7026A-29 | QN7026A-59 on 20240202 end */
//=================================================================================================
/* Added by huofudong for QN7026A-63 | QN7026A-103 on 20240228 begin */
#ifdef SX933X_FACTORY_NODE
...
/* Modify by huofudong for QN7026A-689 on 20240530 end */
/* Added by huofudong for QN7026A-63 | QN7026A-103 on 20240228 end */
...
static void smtc_update_status(Self self)
{
    ...
    /* Added by huofudong for QN7026A-63 on 20240204 begin */
    #ifdef SX933X_ANFR_LOGIC
    ...
    #endif
    /* Added by huofudong for QN7026A-63 on 20240204 end */
...
/* Added by huofudong for QN7026A-192 | QN7026A-682 on 20240529 begin */
//=================================================================================================
...
/* Added by huofudong for QN7026A-192 | QN7026A-682 on 20240529 end */
//#################################################################################################
